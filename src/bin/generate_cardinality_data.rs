...
// Use Arc<str> for interned strings - cheap to clone
...
fn generate_parquet_file(cardinality: usize, output_path: PathBuf) -> std::io::Result<()> {
    ...
    let tag_combinations: Vec<Vec<Arc<str>>> = (0..cardinality)
        .into_par_iter()
        .map(|combo_idx| {
    ...
    println!("  Generating data with interned strings...");

    const TAGSETS_PER_CHUNK: usize = 1000;

    for chunk_start in (0..cardinality).step_by(TAGSETS_PER_CHUNK) {
        let chunk_end = (chunk_start + TAGSETS_PER_CHUNK).min(cardinality);

        if chunk_start > 0 && chunk_start % 10000 == 0 {
            println!(
                "  Progress: {}/{} tagsets ({}%)",
                chunk_start,
                cardinality,
                100 * chunk_start / cardinality
            );
        }
        ...
        for combo_idx in chunk_start..chunk_end {
            ...
            for point_idx in 0..points_per_tagset {
                ...
                for (col_idx, tag_val) in tag_combo.iter().enumerate() {
                    tag_columns[col_idx].push(Some(tag_val.to_string()));
                }

                if timestamps.len() >= ROWS_PER_BATCH {
                    write_batch(&mut writer, &schema, &mut timestamps, &mut tag_columns)?;
...
fn main() {
    ...
    println!("====================================================================");
    println!("GENERATING CARDINALITY TEST DATA");
    println!("====================================================================");
    println!("\nParameters:");
